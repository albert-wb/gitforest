...
*   [Grass.tsx](file:///d:/Projetos%20pessoais/gitforest/src/components/Scene/Grass.tsx):
    *   ~100 mil talos curvados em tiles instanciados, com vento no vertex shader e cor derivada de `terrain.colorAt()`.
    *   **O campo é uma janela que segue a câmera**, não um disco fixo ao redor da origem: uma grade 5x5 de tiles centrada no alvo do `OrbitControls`. Antes bastava arrastar a cena com o botão direito para sair da grama e encontrar terreno pelado, e a única saída seria aumentar o raio — o que multiplica o custo pela **área** para cobrir onde ninguém está olhando.
    *   Isso só é possível porque a seed de cada tile vem das suas coordenadas de grade: um tile gerado ao passar é idêntico ao que seria gerado ao voltar. No máximo dois tiles são construídos por quadro, para o arrasto não engasgar, e o cache é podado em cinquenta tiles.
...
| **v1.4.1** | 2026-08-19 | IA (Claude) | `src/components/Scene/Grass.tsx`, `src/components/Forest/ForestPicking.tsx`, `src/world/countryLayout.ts`, `src/store/useForestStore.ts`, `src/api/forest.ts`, `codebase.md` | **Mais árvores visíveis e grama que acompanha a câmera — tudo medido:**<br>1. **Erro de densidade corrigido em `countryLayout`.** `SPACING` foi tratado como se fosse a distância entre vizinhas; na verdade, com `raio = INNER + S·√i`, a densidade é `1/(πS²)` e a distância média é `1,77·S`. Com `S = 4,4` as árvores ficavam a quase oito unidades de distância e o raio de carga continha **19** árvores, não as 130 previstas — o "limite baixo de árvores visíveis" era esse engano, não uma trava de performance. `S = 2,5`.<br>2. **`ForestPicking` instanciado.** A camada de seleção usava um `<mesh>` por árvore; a medição mostrou a cena saltando de **32 para 99 chamadas de desenho** com setenta árvores. A floresta que custou trabalho para caber em duas passava a gastar setenta em alvos invisíveis. Um `InstancedMesh` com `instanceId` no evento devolve tudo a uma chamada.<br>3. **Limites ampliados sobre número medido.** `LOAD_RADIUS` 28 → 40 (19 → **165** árvores no modo país) e os graus de amizade de 24/48 → 32/80 (72 → 112). Medido: **38 chamadas de desenho, constantes de 56 a 165 árvores**, e ~16,6 mil triângulos por árvore de fundo, somando ~2,9 M na cena cheia. O raio é o botão de ajuste, e ele escala com a **área**: dobrá-lo quadruplica a contagem.<br>4. **Grama passou a ser uma janela que segue a câmera.** Era um campo fixo de raio 50 ao redor da origem, então bastava arrastar a cena para sair dela e achar terreno pelado. Agora é uma grade 5×5 de tiles centrada no alvo do `OrbitControls`, construída no máximo dois tiles por quadro para não engasgar durante o arrasto. Só funciona porque a seed de cada tile vem das suas coordenadas de grade: ir e voltar reencontra a mesma grama.<br>5. **Correções na própria grama:** o centro da grade era inicializado em `[0,0]`, igual à célula da câmera na abertura, então a fila de construção nunca era montada e o campo ficava vazio (`NaN` como sentinela resolve); e o cache de tiles crescia sem teto ao caminhar (~320 KB cada), agora podado a cinquenta pelos mais distantes. |
